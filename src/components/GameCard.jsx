...
function GameCard({ game, onAddToCart, onBuyNow, onOpenDetail, priority = false }) {
  // 🛠️ STATE THEO DÕI LỖI ẢNH
  const [imgError, setImgError] = useState(false);

  // 🛠️ LINK ẢNH DỰ PHÒNG CAO CẤP (Khi ảnh gốc bị die)
  // Bạn có thể thay link này bằng ảnh logo shop của bạn
  const fallbackImage = "https://placehold.co/600x338/080d16/22d3ee?text=IMAGE+UPDATING";
  ...
  return (
    ...
    >
      <article className="relative w-full h-full overflow-hidden rounded-[1rem] md:rounded-[1.6rem] border border-white/5 bg-[#080d16] transition-all duration-300 md:group-hover:-translate-y-1 flex flex-col shadow-lg transform-gpu">
        ...
        {/* 1. ẢNH POSTER & CƠ CHẾ FIX ẢNH DIE */}
        <div className="relative aspect-[16/9] w-full overflow-hidden bg-slate-950 shrink-0">
          <img
            // 🛠️ NẾU LỖI THÌ DÙNG ẢNH DỰ PHÒNG
            src={imgError ? fallbackImage : optimizedPoster}
            alt={game.title}
            loading={priority ? "eager" : "lazy"}
            fetchpriority={priority ? "high" : "auto"}
            decoding="async"
            // 🛠️ KÍCH HOẠT NHẢY ẢNH DỰ PHÒNG KHI CÓ LỖI
        ...
        {/* 2. KHỐI THÔNG TIN */}
        <div className="p-2.5 md:p-4 flex flex-col flex-1 min-w-0 text-left">
        ...
        {/* 3. CỤM NÚT BẤM */}
        <div className="px-2.5 pb-2 md:px-4 md:pb-3 flex flex-col gap-2 shrink-0">
          <div className="flex flex-row items-center gap-2">
            ...
            <button
              ...
              className="flex-1 py-1.5 md:py-2 rounded-full bg-cyan-500/10 border border-cyan-500/20 text-cyan-400 text-[10px] md:text-xs font-black uppercase tracking-widest hover:bg-cyan-500 hover:text-black transition-all active:scale-95 shadow-md shadow-cyan-500/5"
            >
              Nhận ngay
            </button>
          </div>

          <p className="text-[8px] md:text-[9px] text-white/60 font-black tracking-[0.1em] text-center leading-tight">
